...
import { UploadCloud, FilePlus2, FileText, FolderOpen } from "lucide-react";
    ...
    return (
      <div className="flex items-center gap-2">
        <div
          {...getRootProps()}
          className={cn(
            "flex-1 flex items-center justify-center gap-2 rounded-xl border-2 border-dashed px-4 py-3 text-sm cursor-pointer transition",
            isDragActive
        ...
        <button
          ...
          disabled={disabled}
          className={cn(
            "flex items-center gap-2 rounded-xl border-2 border-dashed px-4 py-3 text-sm cursor-pointer transition",
            "border-slate-300 bg-white hover:border-indigo-400 hover:bg-indigo-50/50 text-slate-700",
            disabled && "opacity-50 cursor-not-allowed"
          )}
        >
    ...
    <div className="space-y-3">
      ...
      >
        ...
        <p className="relative text-xs text-slate-500 mt-4 flex items-center gap-2">
          ...
          Múltiplos arquivos · Arraste pastas inteiras
        </p>
      </div>
      ...
      <button
        ...
        disabled={disabled}
        className={cn(
          "w-full flex items-center justify-center gap-2 rounded-xl border-2 border-dashed px-4 py-3 text-sm cursor-pointer transition",
          "border-slate-300 bg-white hover:border-indigo-400 hover:bg-indigo-50/50 text-slate-700",
          disabled && "opacity-50 cursor-not-allowed"
        )}
      >
        <FolderOpen className="h-4 w-4" />
        <span className="font-medium">Ou selecione uma pasta inteira</span>
      </button>
    </div>
  );
}
